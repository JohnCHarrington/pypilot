...
class AsyncIOClient(ABC):
    ...
    and _encode_impl. Transports that do not use asyncio stream writers should
    override _send_impl.
    """
    ...
    async def _send_impl(self, encoded_message):
        """Send one transport-encoded message.

        The default implementation writes to an asyncio stream writer. Subclasses
        with different transport primitives can override this method.
        """
    ...
    def _handle_receive_task_done(self, task: asyncio.Task):
        """Log terminal receive task state so background failures are visible."""
        try:
            exception = task.exception()
        except asyncio.CancelledError:
            self.logger.info("Receive loop task cancelled")
            return

        if exception is not None:
            self.logger.error(
                "Receive loop task failed: %s",
                exception,
                exc_info=(type(exception), exception, exception.__traceback__),
            )
            return

        self.logger.info("Receive loop task terminated")
            ...
            async for attempt in AsyncRetrying(
                ...
                with attempt:
                    ...
                    if self._receive_task and not self._receive_task.done():
                        ...
                        print("Going to cancel existing receive task")
                    ...
                    print("Starting receive loop task")
                    ...
                    print("task:", self._receive_task, "done:", self._receive_task.done(), "cancelled:", self._receive_task.cancelled())
    ...
    async def _receive_loop(self):
        ...
        try:
            ...
            self.logger.info("Received loop cancelled")
            raise
...
class EByteNmea2000Gateway(AsyncIOClient):
    ...
    async def _receive_impl(self):
        ...
        if message is not None:
            ...
            self.logger.debug(f"Message put in queue")
...
class PythonCanAsyncIOClient(AsyncIOClient):
    ...
    async def _connect_impl(self):
        ...
        print(f"Connecting to {self.interface} on {self.channel} with options: {self.can_options}")
        ...
        print(f"Connected to {self.interface} on {self.channel}")
    ...
    @staticmethod
    def _is_transient_send_error(exception: Exception) -> bool:
        """Return True when python-can send failure is caused by TX buffer pressure."""
        transient_errnos = {105}
        current = exception
    ...
    async def _send_impl(self, encoded_message):
        """Send one python-can message through the CAN bus."""
